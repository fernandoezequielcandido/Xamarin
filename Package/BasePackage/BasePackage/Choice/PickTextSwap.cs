...
namespace Laavor
{
    namespace Choice
    {
        ...
        /// Class PickTextShift
        /// </summary>
        public class PickTextSwap : StackLayout
        {
            /// <summary>
            ...
            /// <summary>
            /// Constructor of ChoiceTextShift
            /// </summary>
            public PickTextSwap() : base()
            {
                ValueChosen = "";
                TextChosen = "";
                IndexChosen = -1;
            ...
            public static readonly BindableProperty SpaceBetweenProperty = BindableProperty.Create(
                     ...
                     declaringType: typeof(PickTextSwap),
                     defaultValue: 0,
            ...
            {
                PickTextSwap choiceTextShift = (PickTextSwap)bindable;
                int copySpace = (int)newValue;

                if (choiceTextShift.dataItems != null)
                {
                    for (int iItem = 1; iItem < choiceTextShift.dataItems.Children.Count; iItem++)
                    {
                        if (choiceTextShift.dataItems.Children[iItem].GetType() == typeof(Grid))
                        {
                            Grid grid = (Grid)choiceTextShift.dataItems.Children[iItem];
                            if (grid.Children[0].GetType() == typeof(PickTextBase))
                            {
                                PickTextBase choiceTextBase = (PickTextBase)grid.Children[0];

                                if (choiceTextShift.ChoiceOrientation == ChoiceOrientation.Horizontal)
                                {
            ...
            private static void InitialIndexChosenPropertyChanged(object bindable, object oldValue, object newValue)
            {
                ...
                if (choiceTextShift.dataItems != null)
                {
                    for (int iItem = 0; iItem < choiceTextShift.dataItems.Children.Count; iItem++)
                    {
                        if (choiceTextShift.dataItems.Children[iItem].GetType() == typeof(Grid))
                        {
                            Grid grid = (Grid)choiceTextShift.dataItems.Children[iItem];
                            if (grid.Children[0].GetType() == typeof(PickTextBase))
                            {
                                ...
                                if (choiceTextBase.Index == copyIndex)
                                {
                                    ...
                                    choiceTextShift.IndexChosen = choiceTextBase.Index;
                                    choiceTextShift.TextChosen = choiceTextBase.Text;
                                    choiceTextShift.ValueChosen = choiceTextBase.Value;
                                    choiceTextBase.Clicked += choiceTextShift.ChoiceTextBase_Touched;
                                }
                                else
                                {
            ...
            private static void FontFamilyPropertyChanged(object bindable, object oldValue, object newValue)
            {
                ...
                if (choiceTextShift.dataItems != null)
                {
                    for (int iItem = 0; iItem < choiceTextShift.dataItems.Children.Count; iItem++)
                    {
                        if (choiceTextShift.dataItems.Children[iItem].GetType() == typeof(PickTextBase))
                        {
                            if (choiceTextShift.dataItems.Children[iItem].GetType() == typeof(Grid))
                            {
                                Grid grid = (Grid)choiceTextShift.dataItems.Children[iItem];
            ...
            {
                PickTextSwap choiceTextShift = (PickTextSwap)bindable;
                choiceTextShift.items = (IEnumerable)newValue;
                choiceTextShift.InitAllBindable();
            }
            ...
            private static void HeightPropertyChanged(object bindable, object oldValue, object newValue)
            {
                ...
                if (choiceTextShift.scroll != null)
                {
                    if (choiceTextShift.ChoiceOrientation == ChoiceOrientation.Vertical)
                    {
                        choiceTextShift.scroll.HeightRequest = height;
                    }
                    else
                    {
                        choiceTextShift.scroll.WidthRequest = -1;
                    }
                }
            }
            ...
            private static void WidthPropertyChanged(object bindable, object oldValue, object newValue)
            {
                ...
                if (choiceTextShift.scroll != null)
                {
                    if (choiceTextShift.ChoiceOrientation == ChoiceOrientation.Horizontal)
                    {
                        choiceTextShift.scroll.WidthRequest = width;
                    }
                    else
                    {
                        choiceTextShift.scroll.HeightRequest = -1;
                    }
                }
            }
            ...
            {
                PickTextSwap choiceTextShift = this;
                if (dataItems == null)
                {
                    dataItems = new StackLayout();
                }

                if (choiceTextShift.items != null && !string.IsNullOrEmpty(choiceTextShift.TextChosenField) && !string.IsNullOrEmpty(choiceTextShift.TextChosenField) && !string.IsNullOrEmpty(choiceTextShift.ValueField))
                {
                    dataItems.Children.Clear();
                    listText = new List<PickTextBase>();

                    var enumerator = choiceTextShift.items.GetEnumerator();
                    while (enumerator.MoveNext())
                    {
                        object obj = enumerator.Current;
                        string textUnchosenObj = obj.GetType().GetProperty(choiceTextShift.TextUnChosenField).GetValue(obj).ToString();
                        string textChosenObj = obj.GetType().GetProperty(choiceTextShift.TextChosenField).GetValue(obj).ToString();
                        string valueObj = obj.GetType().GetProperty(choiceTextShift.ValueField).GetValue(obj).ToString();
                        ...
                        item.TextColorChosen = choiceTextShift.TextColorChosen;
                        item.TextColorUnChosen = choiceTextShift.TextColorUnChosen;
                        item.ColorUIChosen = choiceTextShift.ColorUIChosen;
                        item.ColorUIUnChosen = choiceTextShift.ColorUIUnChosen;
                        item.BorderWidth = choiceTextShift.BorderWidth;
                        item.CornerRadius = choiceTextShift.CornerRadius;
                        item.BorderColorChosen = choiceTextShift.BorderColorChosen;
                        item.BorderColorUnChosen = choiceTextShift.BorderColorUnChosen;
                        item.FontSize = choiceTextShift.FontSize;
                        item.FontType = choiceTextShift.FontType;
                        item.FontFamily = choiceTextShift.FontFamily;
                        item.Height = choiceTextShift.HeightBox;
                        item.Width = choiceTextShift.WidthBox;
            ...
            protected override void OnChildAdded(Element child)
            {
                ...
                if (child.GetType() == typeof(PickTextSwapItem))
                {
                    ...
                    else if (InitialIndexChosen == currentIndex)
                    {
                        ...
                        choiceTextBase.BackgroundColor = Colors.Get(ColorUIChosen);
                        IndexChosen = currentIndex;
                        TextChosen = choiceTextShiftItem.TextChosen;
                        ValueChosen = choiceTextShiftItem.Value;
                        choiceTextBase.IsChosen = true;
                    }
                ...
                if (!choiceTextBase.IsChosen)
                {
                    ...
                    if (IndexChosen != -1)
                    {
                        if (dataItems.Children[IndexChosen].GetType() == typeof(Grid))
                        {
                            ...
                            choiceTextPrevious.Text = choiceTextBase.TextUnChosen;
