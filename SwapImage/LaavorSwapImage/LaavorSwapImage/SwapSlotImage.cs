    ...
    public class SwapSlotImage : StackLayout
    {
        ...
        public void Update(string hash, int indexImage, int numberContent, string valueImage, string valueContent, string source)
        {
            if (hash == "laavorffabna87987*--/*-*-/-*.8sdfsakjksfdjkb")
            {
                ...
                {
                    ItemsChosen[numberContent - 1].Update("laavorffabna87987*--/*-*-/-*.8sdfsakjksfdjkb", indexImage, numberContent, valueImage, valueContent, source);
        ...
        private static void ImageWidthChanged(object bindable, object oldValue, object newValue)
        {
            ...
            for (int iItem = 0; iItem < slotImage.Children.Count; iItem++)
            {
                if (slotImage.Children[iItem].GetType() == typeof(SwapSlotContent))
                {
                    ...
                    slotContent.ChangeWidth("laavorffabna87987*--/*-*-/-*.8sdfsakjksfdjkb", copyWidth);
                }
            }
        }
        ...
        private static void ImageHeightChanged(object bindable, object oldValue, object newValue)
        {
            ...
            for (int iItem = 0; iItem < slotImage.Children.Count; iItem++)
            {
                ...
                {
                    SwapSlotContent slotContent = (SwapSlotContent)slotImage.Children[iItem];
                    slotContent.ChangeHeight("laavorffabna87987*--/*-*-/-*.8sdfsakjksfdjkb", copyHeight);
                }
            }
        }
        ...
        protected override void OnChildAdded(Element child)
        {
            ...
            if (child.GetType() == typeof(SwapSlotContent))
            {
                ...
                (child as SwapSlotContent).ChangeHeight("laavorffabna87987*--/*-*-/-*.8sdfsakjksfdjkb", ImageHeight);
                (child as SwapSlotContent).ChangeWidth("laavorffabna87987*--/*-*-/-*.8sdfsakjksfdjkb", ImageWidth);
                ...
                ItemsChosen.Insert(countItems, new ChosenItem("laavorffabna87987*--/*-*-/-*.8sdfsakjksfdjkb", 0, (countItems + 1), slotContent.Value, valueImage, sourceImage));
                slotContent.InformSlotImage("laavorffabna87987*--/*-*-/-*.8sdfsakjksfdjkb", this);
                ...
                (child as SwapSlotContent).InformNumber("laavorffabna87987*--/*-*-/-*.8sdfsakjksfdjkb", countItems);
            }
        }
    }
}
